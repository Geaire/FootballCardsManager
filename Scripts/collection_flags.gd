...
var current_page:int=1; var is_animating:bool=false; var country_progress:Dictionary={}

@onready var page1=$Page1; @onready var page2=$Page2
@onready var swipe_down=$Page1/Swipe_Down; @onready var swipe_up=$Page2/Swipe_Up

func _ready():
	page2.position.y=1080.0; _load_progress()
...
func _on_progress_loaded(data:Dictionary):
	if Firebase.firestore_success.is_connected(_on_progress_loaded): Firebase.firestore_success.disconnect(_on_progress_loaded)
	if Firebase.firestore_failed.is_connected(_on_progress_failed):  Firebase.firestore_failed.disconnect(_on_progress_failed)
	for code in PAGE1_COUNTRIES+PAGE2_COUNTRIES: country_progress[code]=int(data.get(code,0))
	_update_counters()
...
func _input(event):
	...
	for code in countries:
		...
		if flag and _sprite_hit(flag,pos):
			GameState.selected_country=code; GameState.previous_scene="res://Scenes/collection_flags.tscn"
			get_tree().change_scene_to_file(SCENE_COLLECTION_COUNTRY); return
