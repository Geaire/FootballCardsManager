...
func _populate_card_grid(cards: Array):
	...
	for card_data in cards:
		...
		var card = cs.instantiate() as Control
...
func _on_press(pos: Vector2):
	...
	if _panel_hit(btn_bonus1, pos): pass; return
	if _panel_hit(btn_bonus2, pos): pass; return
...
func _on_release(pos: Vector2):
	...
	if popup_cards.visible and not drag_active and press_card_index >= 0 and was_holding:
		for i in range(popup_card_nodes.size()):
			if popup_card_nodes[i].get_global_rect().has_point(pos) and i == press_card_index:
				_open_dcp_from_popup(i)
				press_card_index = -1
				return
		press_card_index = -1
		return
...
# ── HELPERS ───────────────────────────────────────────────────────────────────
func _set_panel_color(panel: Control, c: Color):
...
func _panel_hit(panel: Control, pos: Vector2) -> bool:
