...
const PIN_COLORS = {
	"cyan":     Color(0.0,   0.808, 0.820), "marine":   Color(0.0,   0.200, 0.400),
	"rose":     Color(1.0,   0.412, 0.706), "violet":   Color(0.416, 0.051, 0.678),
	"gris":     Color(0.722, 0.722, 0.722), "marron":   Color(0.545, 0.271, 0.075),
	"rouge":    Color(1.0,   0.420, 0.420), "bordeaux": Color(0.502, 0.0,   0.125),
	"vertclair":Color(0.584, 0.835, 0.698), "vertfonce":Color(0.176, 0.416, 0.310)
}
...
func _ready():
	...
	btn_sound_on.visible = GameState.sound_on; btn_sound_off.visible = not GameState.sound_on
...
func _input(event):
	...
	if _sprite_hit(btn_play, pos):
		play_click_sound(); await get_tree().create_timer(0.2).timeout
