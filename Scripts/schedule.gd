...
const COMP_COLORS = {
	"championnat_classique": Color(0.0,   0.200, 0.400), "coupe_classique":      Color(0.0,   0.808, 0.820),
	"championnat_pantheon":  Color(0.416, 0.051, 0.678), "coupe_pantheon":       Color(1.0,   0.412, 0.706),
	"championnat_jeunes":    Color(0.545, 0.271, 0.075), "coupe_jeunes":         Color(0.722, 0.722, 0.722),
	"championnat_asso":      Color(0.502, 0.0,   0.125), "coupe_asso":           Color(1.0,   0.420, 0.420),
	"championnat_special":   Color(0.176, 0.416, 0.310), "autre":                Color(0.584, 0.835, 0.698),
}
...
const CATALOGUE_TRANSLATIONS = {
	...
	"championnat_asso":      {"fr":"Champ. Asso","en":"Assoc. Champ.","es":"Camp. Asoc.","de":"Vereins Meistersch.","it":"Camp. Assoc.","pt":"Camp. Assoc."},
	"coupe_asso":            {"fr":"Coupe Asso","en":"Assoc. Cup","es":"Copa Asoc.","de":"Vereinspokal","it":"Coppa Assoc.","pt":"Taça Assoc."},
	"championnat_special":   {"fr":"Champ. Spécial","en":"Special Champ.","es":"Camp. Especial","de":"Spezial Meistersch.","it":"Camp. Speciale","pt":"Camp. Especial"},
	"autre":                 {"fr":"Matchs Amicaux","en":"Friendly Matches","es":"Amistosos","de":"Freundschaftsspiele","it":"Amichevoli","pt":"Amigáveis"},
}

const WEEKLY_CATALOGUE = [
	["championnat_classique"],["coupe_classique"],["championnat_pantheon"],
	["coupe_pantheon"],["championnat_jeunes"],["coupe_jeunes"],["autre"],
]
...
@onready var txt_news1              = $Page2/TXT_News1; @onready var txt_news2 = $Page2/TXT_News2
@onready var txt_news3              = $Page2/TXT_News3; @onready var txt_news4 = $Page2/TXT_News4
...
@onready var txt_bonus              = $Page2/TXT_Bonus; @onready var txt_feat_comp = $Page2/TXT_Competitions
@onready var txt_design             = $Page2/TXT_Design; @onready var txt_others = $Page2/TXT_Others
@onready var txt_bugs               = $Page2/TXT_Bugs

var shot_nodes:Array=[]; var comp_labels:Array=[]

func _ready():
	for i in range(1,9): shot_data[i]={}; shot_nodes.append(get_node("Shot_%d"%i))
	for i in range(WEEKLY_CATALOGUE.size()): catalogue_available.append(true)
	for i in range(1,WEEKLY_CATALOGUE.size()+1): comp_labels.append(catalogue.get_node("TXT_Competition%d"%i))
	txt_popup_competitions.visible=false; txt_popup_evolution.visible=false
	_setup_shot_times(); _setup_catalogue(); _setup_news(); _apply_translations()
	_refresh_shots(); _check_lock()
	page1_nodes=[txt_title_competitions,btn_help_competitions,swipe_down,catalogue]
	for i in range(1,9): page1_nodes.append(shot_nodes[i-1])

func _setup_shot_times():
	for i in range(1,9): shot_nodes[i-1].get_node("TXT_Time").text=SHOT_TIMES[i]

func _setup_catalogue():
	var lang=GameState.language
	for i in range(comp_labels.size()): comp_labels[i].visible=false
	for i in range(WEEKLY_CATALOGUE.size()):
		var ct=WEEKLY_CATALOGUE[i][0]
		comp_labels[i].text=CATALOGUE_TRANSLATIONS[ct].get(lang,CATALOGUE_TRANSLATIONS[ct]["en"])
		comp_labels[i].modulate=COMP_COLORS[ct]; comp_labels[i].visible=catalogue_available[i]
...
func _apply_translations():
	...
	txt_title_competitions.text=t["title_competitions"]; txt_title_newsletter.text=t["news_title"]
	txt_title_evolution.text="Football Cards Manager Evolution"
	txt_bonus.text=t["bonus"]; txt_feat_comp.text=t["competitions"]
	txt_design.text=t["design"]; txt_others.text=t["others"]; txt_bugs.text=t["bugs"]
	txt_popup_competitions.text=t["popup_comp"]; txt_popup_evolution.text=t["popup_evo"]

func _refresh_shots():
	for i in range(1,9):
		var txt_c=shot_nodes[i-1].get_node("TXT_Competition"); var bg=shot_nodes[i-1].get_node("Card_Background_Shot")
		if shot_data[i].is_empty(): txt_c.text=""; bg.modulate=Color(1,1,1)
		else: txt_c.text=shot_data[i]["name"]; bg.modulate=COMP_COLORS[shot_data[i]["type"]]
	_setup_catalogue(); _update_shot_availability()

func _update_shot_availability():
	if not drag_active or drag_comp_type not in CHAMPIONSHIP_TYPES:
		for i in range(1,9): shot_nodes[i-1].modulate=Color(1,1,1); return
	for i in range(1,9):
...
func _check_lock():
	...
	for i in range(1,9): shot_nodes[i-1].get_node("OVL_Lock").visible=locked
...
func _process(delta):
	if press_holding and press_slot>0:
		...
		if press_timer>=LONG_PRESS_DURATION:
			...
			if not locked:
				var slot=press_slot; press_slot=-1; _end_drag(); _clear_shot(slot); _refresh_shots()

func _animate_to_page(page:int):
	if is_animating: return
	is_animating=true; txt_popup_competitions.visible=false; txt_popup_evolution.visible=false
	var tween=create_tween(); tween.set_ease(Tween.EASE_IN_OUT); tween.set_trans(Tween.TRANS_CUBIC)
	if page==2:
		current_page=2; for n in page1_nodes: n.visible=false
		tween.tween_property(page2,"position:y",-1080.0,SLIDE_DURATION)
		tween.tween_callback(func(): is_animating=false)
	else:
		current_page=1; tween.tween_property(page2,"position:y",0.0,SLIDE_DURATION)
...
func _input(event):
	if event is InputEventMouseButton and event.button_index==MOUSE_BUTTON_LEFT:
		...
		if press_holding and event.position.distance_to(touch_start)>10: press_holding=false; press_timer=0.0
		if drag_active and drag_visual: drag_visual.position=event.position-Vector2(drag_visual.size.x/2,drag_visual.size.y/2)

func _on_press(pos:Vector2):
	if txt_popup_competitions.visible and _label_hit(txt_popup_competitions,pos): txt_popup_competitions.visible=false; return
	if txt_popup_evolution.visible and _label_hit(txt_popup_evolution,pos): txt_popup_evolution.visible=false; return
	if _sprite_hit(swipe_down,pos) and current_page==1: _animate_to_page(2); return
	if _sprite_hit(swipe_up,pos) and current_page==2: _animate_to_page(1); return
	if _sprite_hit(btn_help_competitions,pos) and current_page==1: txt_popup_competitions.visible=true; return
	if _sprite_hit(btn_help_evolution,pos) and current_page==2: txt_popup_evolution.visible=true; return
	if current_page==2:
	...
	for i in range(1,9):
		if _shot_hit(i,pos) and not shot_data[i].is_empty():
			...
			_start_drag(shot_data[i]["name"],shot_data[i]["type"],i,shot_data[i].get("cat_idx",-1)); return
	for i in range(WEEKLY_CATALOGUE.size()):
		if catalogue_available[i] and _label_hit(comp_labels[i],pos):
			_start_drag(comp_labels[i].text,WEEKLY_CATALOGUE[i][0],-1,i); return
...
func _on_release(pos:Vector2):
	...
	if drag_origin_shot>0 and _catalogue_hit(pos): _clear_shot(drag_origin_shot); _end_drag(); _refresh_shots(); return
	_end_drag()
...
func _start_drag(comp_name:String,comp_type:String,origin_shot:int,origin_cat:int):
	...
	drag_visual=Label.new(); drag_visual.text=comp_name; drag_visual.modulate=COMP_COLORS[comp_type]
	add_child(drag_visual); _update_shot_availability()

func _end_drag():
	drag_active=false; drag_comp_name=""; drag_comp_type=""; drag_origin_shot=-1; drag_origin_cat=-1
	if drag_visual: drag_visual.queue_free(); drag_visual=null
	_update_shot_availability()

func _drop_on_shot(slot:int):
	if not shot_data[slot].is_empty() and slot!=drag_origin_shot: _end_drag(); return
	if drag_comp_type in CHAMPIONSHIP_TYPES:
		...
		if drag_origin_shot>0 and drag_origin_shot!=slot:
			...
			shot_data[drag_origin_shot]={}; if old_pair>0: shot_data[old_pair]={}
		...
		if drag_origin_shot>0 and drag_origin_shot!=slot: shot_data[drag_origin_shot]={}
		shot_data[slot]={"name":drag_comp_name,"type":drag_comp_type,"cat_idx":drag_origin_cat}
		if drag_origin_cat>=0: catalogue_available[drag_origin_cat]=false
...
func _clear_shot(slot:int):
	...
	var t=shot_data[slot].get("type",""); var cat_idx=shot_data[slot].get("cat_idx",-1)
	if t in CHAMPIONSHIP_TYPES:
		if cat_idx>=0:
			var pair=CHAMPIONSHIP_PAIRS.get(slot,-1); if pair>0: shot_data[pair]={}
		else:
			for k in CHAMPIONSHIP_PAIRS:
				if CHAMPIONSHIP_PAIRS[k]==slot:
					var mc=shot_data[k].get("cat_idx",-1); if mc>=0: catalogue_available[mc]=true; shot_data[k]={}
	if cat_idx>=0: catalogue_available[cat_idx]=true; shot_data[slot]={}
