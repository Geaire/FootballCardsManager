...
# ── GÉNÉRATION ────────────────────────────────────────────────────────────────
func generate():
	generate_note_and_color(); generate_age_height_weight()
	generate_nationality();    generate_positions()
	generate_name();           generate_specialties()
	generate_skills();         _generate_id()
...
func _input(event):
	...
	if bg_card_background.get_global_rect().has_point(pos):
		_on_card_clicked()
