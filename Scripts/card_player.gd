...
@onready var card_background = $BG_CardBackground
...
@onready var img_soccerball  = $IMG_SoccerBall
...
func _ready():
	if get_parent() == get_tree().root:
		position = get_viewport_rect().size / 2; generate(); display()
...
func save_to_firestore():
	...
	Firebase.update_document(
		...
		selected_card_id, {
			...
			"firstname": firstname, "lastname": lastname, "pin_color": pin_color,
...
func generate_positions():
	...
	if position1 == "GB": position2 = ""; return
	if randi_range(1, 100) <= 10:
		var candidates = []
		for p in POSITIONS:
			if p != position1 and p != "GB": candidates.append(p)
...
func generate_name():
	...
	if file == null: firstname = "John"; lastname = "Doe"; return
	var json = JSON.parse_string(file.get_as_text()); file.close()
	if typeof(json) != TYPE_DICTIONARY: firstname = "John"; lastname = "Doe"; return
...
func generate_specialties():
	...
	var roll = randi_range(1, 100)
	if roll <= 90:   specialty1 = ""; specialty2 = ""
	elif roll <= 99: specialty1 = pool[randi_range(0, pool.size() - 1)]; specialty2 = ""
	else:
		specialty1 = pool[randi_range(0, pool.size() - 1)]
		if pool.size() > 1:
			var s2 = specialty1
			while s2 == specialty1: s2 = pool[randi_range(0, pool.size() - 1)]
			specialty2 = s2
		else: specialty2 = ""
...
func display():
	...
	txt_note.text = str(note); txt_position1.text = position1
	if position2 != "" and position2_unlocked == 1:
		txt_position2.text = position2; txt_position2.visible = true
	else: txt_position2.visible = false
	txt_firstname.text = firstname; txt_lastname.text = lastname
	# Pin color — invisible par defaut, visible seulement si pin attribuee
	if pin_color != "" and pin_color in PIN_COLORS:
		btn_pincolor.modulate = PIN_COLORS[pin_color]
		btn_pincolor.z_index  = 5
		btn_pincolor.visible  = true
	else:
		btn_pincolor.visible  = false
		btn_pincolor.z_index  = 3
	img_soccerball.z_index = 4
	# Specialites — Sprite2D icones
	...
	img_flag.texture = tex; img_flag.visible = tex != null

func _input(event):
	if not (event is InputEventMouseButton): return
	if not (event.button_index == MOUSE_BUTTON_LEFT and event.pressed): return
	var pos = event.position
	# Clic sur BTN_PinColor (visible = pin active) → supprimer la pin color
	if btn_pincolor.visible and _sprite_hit(btn_pincolor, pos):
		pin_color = ""
		display()
		# Sync vers GameState selon le deco_index
		if deco_index == 1: GameState.deco1_pin_color = ""
		elif deco_index == 2: GameState.deco2_pin_color = ""
		return
	# Clic sur la carte → ouvrir DCP
	if card_background.get_rect().has_point(to_local(pos)):
		_on_card_clicked()

func _on_card_clicked():
	if not clickable: return
	GameState.selected_note = note;                    GameState.selected_color = color
	GameState.selected_position1 = position1;         GameState.selected_position2 = position2
	GameState.selected_position2_unlocked = position2_unlocked
	GameState.selected_age = age;                      GameState.selected_height = height
	GameState.selected_weight = weight;                GameState.selected_nationality = nationality
	GameState.selected_specialty1 = specialty1;       GameState.selected_specialty2 = specialty2
	GameState.selected_firstname = firstname;          GameState.selected_lastname = lastname
	GameState.selected_pin_color = pin_color;          GameState.selected_strength = strength
	GameState.selected_speed = speed;                  GameState.selected_aggression = aggression
	GameState.selected_positioning = positioning;      GameState.selected_stamina = stamina
	GameState.selected_creativity = creativity;        GameState.selected_concentration = concentration
	GameState.selected_motivation = motivation;        GameState.selected_anticipation = anticipation
	GameState.selected_communication = communication;  GameState.selected_deco_index = deco_index
...
func save_to_gamestate_deco1():
	GameState.deco1_note=note; GameState.deco1_color=color; GameState.deco1_position1=position1
	GameState.deco1_position2=position2; GameState.deco1_position2_unlocked=position2_unlocked
	...
	GameState.deco1_lastname=lastname; GameState.deco1_pin_color=pin_color
	GameState.deco1_strength=strength; GameState.deco1_speed=speed; GameState.deco1_aggression=aggression
	GameState.deco1_positioning=positioning; GameState.deco1_stamina=stamina
	GameState.deco1_creativity=creativity; GameState.deco1_concentration=concentration
	GameState.deco1_motivation=motivation; GameState.deco1_anticipation=anticipation
	GameState.deco1_communication=communication

func save_to_gamestate_deco2():
	GameState.deco2_note=note; GameState.deco2_color=color; GameState.deco2_position1=position1
	GameState.deco2_position2=position2; GameState.deco2_position2_unlocked=position2_unlocked
	...
	GameState.deco2_lastname=lastname; GameState.deco2_pin_color=pin_color
	GameState.deco2_strength=strength; GameState.deco2_speed=speed; GameState.deco2_aggression=aggression
	GameState.deco2_positioning=positioning; GameState.deco2_stamina=stamina
	GameState.deco2_creativity=creativity; GameState.deco2_concentration=concentration
	GameState.deco2_motivation=motivation; GameState.deco2_anticipation=anticipation
	GameState.deco2_communication=communication

func restore_from_gamestate_deco1():
	note=GameState.deco1_note; color=GameState.deco1_color; position1=GameState.deco1_position1
	position2=GameState.deco1_position2; position2_unlocked=GameState.deco1_position2_unlocked
	...
	lastname=GameState.deco1_lastname; pin_color=GameState.deco1_pin_color
	strength=GameState.deco1_strength; speed=GameState.deco1_speed; aggression=GameState.deco1_aggression
	positioning=GameState.deco1_positioning; stamina=GameState.deco1_stamina
	creativity=GameState.deco1_creativity; concentration=GameState.deco1_concentration
	motivation=GameState.deco1_motivation; anticipation=GameState.deco1_anticipation
	communication=GameState.deco1_communication

func restore_from_gamestate_deco2():
	note=GameState.deco2_note; color=GameState.deco2_color; position1=GameState.deco2_position1
	position2=GameState.deco2_position2; position2_unlocked=GameState.deco2_position2_unlocked
	...
	lastname=GameState.deco2_lastname; pin_color=GameState.deco2_pin_color
	strength=GameState.deco2_strength; speed=GameState.deco2_speed; aggression=GameState.deco2_aggression
	positioning=GameState.deco2_positioning; stamina=GameState.deco2_stamina
	creativity=GameState.deco2_creativity; concentration=GameState.deco2_concentration
	motivation=GameState.deco2_motivation; anticipation=GameState.deco2_anticipation
