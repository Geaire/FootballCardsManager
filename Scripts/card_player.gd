...
var clickable: bool = true;  var deco_index: int = 0
...
func generate_note_and_color():
	...
	if roll <= 400:
		note = randi_range(60, 69);   color = "yellow"
	elif roll <= 650:
		note = randi_range(70, 79);   color = "orange"
	elif roll <= 850:
		note = randi_range(80, 89);   color = "red"
	elif roll <= 970:
		note = randi_range(90, 99);   color = "magenta"
	else:
		note = randi_range(100, 129); color = "blue"
...
func generate_age_height_weight():
	...
	weight = int(round((randi_range(195, 245) / 10.0) * (height / 100.0) * (height / 100.0)))
...
func generate_specialties():
	# Spécialités obtenues via récompenses uniquement — jamais à la génération
	specialty1 = ""
	specialty2 = ""

func generate_skills():
	var base = int(note / 10.0)
	var rem  = note - (base * 10)
	strength      = base; speed         = base; aggression  = base
	positioning   = base; stamina       = base; creativity  = base
	concentration = base; motivation    = base; anticipation = base
	communication = base
...
func display():
	...
	lbl_position1.text = GameState.get_pos_label(position1)
	if position2 != "" and position2_unlocked == 1:
		lbl_position2.text    = GameState.get_pos_label(position2)
	...
	_display_specialty(img_specialty1, specialty1)
	_display_specialty(img_specialty2, specialty2)
...
func _display_ball_sprite():
	if ball_color == "":
		img_ball_color.visible = false; return
	var tex = GameState.ball_textures.get(ball_color, null)
	if tex:
		img_ball_color.texture = tex; img_ball_color.visible = true
	else:
		img_ball_color.visible = false

func _display_specialty(img: Sprite2D, key: String):
	if key == "":
		img.visible = false; return
	var tex = GameState.specialty_textures.get(key, null)
	if tex:
		img.texture = tex; img.visible = true
	else:
		var p = "res://Sprites/Specialties/specialty_" + key + ".png"
		if ResourceLoader.exists(p):
			img.texture = load(p); img.visible = true
		else:
			img.visible = false
...
func _input(event):
	...
	if bg_card_background.get_global_rect().has_point(event.position):
		_on_card_clicked()
