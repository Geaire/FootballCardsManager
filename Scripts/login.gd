...
func _apply_translations():
	...
	txt_sign_google.text = t["google"]; txt_sign_email.text = t["email"]; txt_play_guest.text = t["guest"]
...
func _input(event):
	...
	if _sprite_hit(card_email,  pos): get_tree().change_scene_to_file(SCENE_LOGIN_EMAIL); return
	if _sprite_hit(card_guest,  pos): Firebase.sign_in_anonymous(); return
